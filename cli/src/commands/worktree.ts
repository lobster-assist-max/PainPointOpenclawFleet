...
const WORKTREE_NAME_PREFIX = "fleet-";
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.fleet"));
...
async function seedWorktreeDatabase(input: {
  ...
  const sourceEnvFile = resolveFleetEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readFleetEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://fleet:fleet@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "fleet");
    const targetConnectionString = `postgres://fleet:fleet@127.0.0.1:${targetHandle.port}/fleet`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  loadFleetEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Fleet inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Fleet instance helpers");
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Fleet instance")

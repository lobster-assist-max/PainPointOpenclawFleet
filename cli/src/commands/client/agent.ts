...
function buildAgentEnvExports(input: {
  ...
  return [
    `export FLEET_API_URL='${escaped(input.apiBase)}'`,
    `export FLEET_COMPANY_ID='${escaped(input.companyId)}'`,
    `export FLEET_AGENT_ID='${escaped(input.agentId)}'`,
    `export FLEET_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Fleet skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Fleet skills into ~/.codex/skills and ~/.claude/skills",
      )

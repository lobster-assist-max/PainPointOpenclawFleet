...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://fleet:fleet@127.0.0.1:${port}/fleet`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  const defaultDir = resolveDefaultBackupDir(resolveFleetInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "fleet";

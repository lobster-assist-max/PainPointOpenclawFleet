...
const DEFAULT_AGENT_JWT_ISSUER = "fleet";
const DEFAULT_AGENT_JWT_AUDIENCE = "fleet-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveFleetInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveFleetInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" fleet env ")));
...
function collectDeploymentEnvRows(config: FleetConfig | null, configPath: string): EnvVarRow[] {

...
function createTempConfigPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "fleet-allowed-hostname-"));
...
function writeBaseConfig(configPath: string) {
  const base: FleetConfig = {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/fleet-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/fleet-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/fleet-logs",
    },
    server: {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/fleet-storage" },
      s3: {
        bucket: "fleet",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/fleet-secrets/master.key" },
    },
  };
...
describe("allowed-hostname command", () => {
  it("adds and normalizes hostnames", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf-8")) as FleetConfig;

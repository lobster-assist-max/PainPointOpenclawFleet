...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "fleet", LOGNAME: "fleet", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "fleet" }),
      },
    );

    expect(tokens).toEqual(["fleet", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "fleet" }, {
        userInfo: () => ({ username: "fleet" }),
      });

      expect(tokens).toEqual(["fleet", "custom-token"]);
    } finally {

...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.fleetApiUrl)) {
    const legacyUrl = nonEmptyTrimmedString(input.fleetApiUrl) ?? nonEmptyTrimmedString(input.paperclipApiUrl) ?? nonEmptyTrimmedString(merged.paperclipApiUrl);
    if (legacyUrl) merged.fleetApiUrl = legacyUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    fleetApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.fleetApiUrl) ?? nonEmptyTrimmedString(defaults.paperclipApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawFleetApiUrl =
    typeof defaults.fleetApiUrl === "string"
      ? defaults.fleetApiUrl.trim()
      : typeof defaults.paperclipApiUrl === "string"
        ? defaults.paperclipApiUrl.trim()
        : "";
  if (rawFleetApiUrl) {
    try {
      const parsedFleetApiUrl = new URL(rawFleetApiUrl);
      if (
        parsedFleetApiUrl.protocol !== "http:" &&
        parsedFleetApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_fleet_api_url_protocol",
          level: "warn",
          message: `fleetApiUrl must use http:// or https:// (got ${parsedFleetApiUrl.protocol}).`
        });
      } else {
        normalized.fleetApiUrl = parsedFleetApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_fleet_api_url_configured",
          level: "info",
          message: `fleetApiUrl set to ${parsedFleetApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_fleet_api_url_invalid",
        level: "warn",
        message: `Invalid fleetApiUrl: ${rawFleetApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Fleet host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Fleet is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm fleet allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/fleet-claimed-api-key.json and load FLEET_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: fleetApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Fleet hostname is reachable and allowed via `pnpm fleet allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Fleet API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Fleet OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          fleetApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Fleet generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "fleetApiUrl": "https://fleet-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Fleet before key claim is allowed.
    ...
    - ~/.openclaw/workspace/fleet-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/fleet-claimed-api-key.json

    And set the FLEET_API_KEY and FLEET_API_URL in your environment variables as specified here:
    ...
    Then set FLEET_API_KEY and FLEET_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Fleet skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your FLEET_API_URL to the top of your skill and note the path to your FLEET_API_URL
    ...
    ${
      ...
      "Ensure Fleet is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Fleet base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.fleetApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm fleet allowed-hostname <host>
      - then restart Fleet and retry onboarding.
    `);
  }
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              fleetApiUrl: req.body.fleetApiUrl ?? req.body.paperclipApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.fleetApiUrl &&
          !persistedDefaults.fleetApiUrl
        ) {
          missingPersistedFields.push("fleetApiUrl");
        }

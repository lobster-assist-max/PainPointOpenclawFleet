const FAVICON_BLOCK_START = "<!-- FLEET_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- FLEET_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- FLEET_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- FLEET_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.FLEET_IN_WORKTREE) || isTruthyEnvValue(env.PAPERCLIP_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.FLEET_WORKTREE_NAME) ?? nonEmpty(env.PAPERCLIP_WORKTREE_NAME) ?? nonEmpty(env.PAPERCLIP_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.FLEET_WORKTREE_COLOR) ?? normalizeHexColor(env.PAPERCLIP_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="fleet-worktree-enabled" content="true" />',
    `<meta name="fleet-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="fleet-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="fleet-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}

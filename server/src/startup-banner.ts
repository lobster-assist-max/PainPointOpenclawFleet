...
import { resolveFleetConfigPath, resolveFleetEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = (process.env.FLEET_AGENT_JWT_SECRET ?? process.env.PAPERCLIP_AGENT_JWT_SECRET)?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const raw = parsed.FLEET_AGENT_JWT_SECRET ?? parsed.PAPERCLIP_AGENT_JWT_SECRET;
    const fileValue = typeof raw === "string" ? raw.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm fleet onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveFleetEnvPath();

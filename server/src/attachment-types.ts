 ...
 * `FLEET_ALLOWED_ATTACHMENT_TYPES` environment variable to a
 * comma-separated list of MIME types or wildcard patterns to expand the
 * allowed set. (Also accepts legacy `PAPERCLIP_ALLOWED_ATTACHMENT_TYPES`.)
 *
 * Examples:
 *   FLEET_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf
 *   FLEET_ALLOWED_ATTACHMENT_TYPES=image/*,application/pdf,text/*
 *
...
const allowedPatterns: string[] = parseAllowedTypes(
  process.env.FLEET_ALLOWED_ATTACHMENT_TYPES ?? process.env.PAPERCLIP_ALLOWED_ATTACHMENT_TYPES,
);
...
export const MAX_ATTACHMENT_BYTES =
  Number(process.env.FLEET_ATTACHMENT_MAX_BYTES ?? process.env.PAPERCLIP_ATTACHMENT_MAX_BYTES) || 10 * 1024 * 1024;

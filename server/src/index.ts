...
import { bootstrapFleet, shutdownFleet } from "./fleet-bootstrap.js";
...
export async function startServer(): Promise<StartedServer> {
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@fleet.local";
  ...
  if (config.databaseUrl) {
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://fleet:fleet@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "fleet");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://fleet:fleet@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "fleet");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: fleet");
    }

    const embeddedConnectionString = `postgres://fleet:fleet@127.0.0.1:${port}/fleet`;
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.FLEET_AGENT_JWT_SECRET?.trim() ?? process.env.PAPERCLIP_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or FLEET_AGENT_JWT_SECRET) to be set",
      );
    }
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Fleet server failed to start");
    process.exit(1);
  });
}

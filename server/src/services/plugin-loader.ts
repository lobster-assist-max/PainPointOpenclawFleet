 ...
 *    (`~/.fleet/plugins/`) and `node_modules` for packages matching
 *    the `fleet-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Fleet plugins.
 * Packages matching this pattern are considered Fleet plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "fleet-plugin-";
...
export type PluginSource =
  | "local-filesystem"  // ~/.fleet/plugins/ local directory
  | "npm"               // npm packages matching fleet-plugin-* convention
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the fleet-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "fleet-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginLoader {
  ...
  /**
   * Discover Fleet plugins installed as npm packages in the current
   * Node.js environment matching the "fleet-plugin-*" naming convention.
   *
   ...
   * Throws if the package is a Fleet plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
...
/**
 * Check whether a package name matches the Fleet plugin naming convention.
 * Accepts both the "fleet-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @painpoint/plugin-*
  if (name.includes("/")) {
 ...
 *   packageName: "fleet-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Fleet plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a Fleet plugin.
   */

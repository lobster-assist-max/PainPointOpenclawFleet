...
function buildWorkspaceCommandEnv(input: {
  ...
  env.FLEET_WORKSPACE_CWD = input.worktreePath;
  env.FLEET_WORKSPACE_PATH = input.worktreePath;
  env.FLEET_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.FLEET_WORKSPACE_BRANCH = input.branchName;
  env.FLEET_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.FLEET_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.FLEET_WORKSPACE_SOURCE = input.base.source;
  env.FLEET_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.FLEET_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.FLEET_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.FLEET_PROJECT_ID = input.base.projectId ?? "";
  env.FLEET_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.FLEET_AGENT_ID = input.agent.id;
  env.FLEET_AGENT_NAME = input.agent.name;
  env.FLEET_COMPANY_ID = input.agent.companyId;
  env.FLEET_ISSUE_ID = input.issue?.id ?? "";
  env.FLEET_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.FLEET_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.FLEET_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.FLEET_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.FLEET_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.FLEET_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.FLEET_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.FLEET_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.FLEET_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.FLEET_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.FLEET_PROJECT_ID = input.workspace.projectId ?? "";
  env.FLEET_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.FLEET_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".fleet", "worktrees");

...
Fleet V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Fleet-shaped design.

Assumption for this document: Fleet is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Fleet objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Fleet.

The main conclusion is:

- Fleet should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Fleet should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Fleet should use multiple extension classes instead of one generic plugin bag:
...
If Fleet does this well, the examples you listed become straightforward:
...
Relevant Fleet files reviewed for current extension seams:
...
It is too dangerous for Fleet core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Fleet — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Fleet should absolutely do this.
...
Fleet should copy this discipline.
...
Fleet should adopt this style for plugin actions, automations, and UI schemas.
...
Fleet can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Fleet too.

## What Fleet Should Not Copy Directly
...
Fleet is a control plane for an operator-managed instance with company objects.
...
Fleet is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Fleet should not auto-load arbitrary code from a workspace repo like `.paperclip/plugins` or project directories.
...
For Fleet, equivalent hooks into:
...
Fleet should not allow plugins to silently replace:
...
For Fleet it would be risky because it combines:
...
Fleet should require an explicit operator install step.

## Why Fleet Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Fleet |
|---|---|---|
...
Fleet has several extension-like seams already:
...
This is good news.
Fleet does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Fleet Plugin Model
...
Fleet already has this pattern implicitly:
...
- `plugin` means integration code that talks to Fleet through a typed plugin protocol and capability model
...
- invoke explicit Fleet actions through the API
...
Fleet should treat plugin installation as a global instance-level action.

Examples:

- install `@paperclip/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Fleet objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Fleet already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Fleet too.
...
Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Fleet server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Fleet's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For Fleet
...
If it needs mappings over specific Fleet objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of Fleet itself
...
If a concept is becoming part of Fleet's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Fleet itself
...
### 3. Scope plugin data to Fleet objects before adding custom schemas

A lot of plugin data naturally hangs off existing Fleet objects:
...
If Fleet eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Fleet.
- Board reviews the diff after a run without leaving Fleet.
...
This plugin syncs Fleet work with Linear. It is useful for:

- importing backlog from Linear
- linking Fleet issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Fleet and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Fleet.
- Fleet issue status changes push to Linear and Linear comments arrive back through webhooks.
...
This plugin syncs Fleet issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Fleet issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Fleet task governance
...
- Plugin settings:
  ...
  - whether new Fleet issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Fleet issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Fleet.
- GitHub webhooks update status/comment state in Fleet.
- A PR is linked back to the Fleet issue so the board can follow delivery status.
...
This plugin surfaces external metrics and dashboards inside Fleet. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Fleet issues from anomalous metrics
...
- Board sees service degradation or business KPI movement directly on the Fleet dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Fleet issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Fleet core; keep it as additive signal and issue creation
...
This plugin pulls Stripe revenue and subscription data into Fleet. It is useful for:
...
- Failed payment spikes or churn events can generate Fleet issues for follow-up.
...
- Stripe data should stay additive to Fleet core
...
Fleet should not implement "an OpenCode-style generic in-process hook system."
Fleet should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Fleet

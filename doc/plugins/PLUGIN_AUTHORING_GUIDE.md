...
This guide describes the current, implemented way to create a Fleet plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Fleet app.
...
For local development, install it into Fleet from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.

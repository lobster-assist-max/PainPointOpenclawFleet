# Fleet Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Fleet's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Fleet app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Fleet HTTP APIs directly.
...
Fleet plugin design is based on the following assumptions:

1. Fleet is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Fleet business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Fleet core.
...
2. Let plugins add major capabilities without editing Fleet core.
...
The single Fleet deployment an operator installs and controls.

### 5.2 Company

A first-class Fleet business object inside the instance.
...
A trusted in-process extension loaded directly by Fleet core.
...
An installable instance-wide extension package loaded through the Fleet plugin runtime.
...
Fleet has two extension classes.
...
Fleet already has a concrete workspace model:
...
Plugins live under the Fleet instance directory.
...
Fleet should add CLI commands:

- `pnpm fleet plugin list`
- `pnpm fleet plugin install <package[@version]>`
- `pnpm fleet plugin uninstall <plugin-id>`
- `pnpm fleet plugin upgrade <plugin-id> [version]`
- `pnpm fleet plugin doctor <plugin-id>`
...
Plugins may contribute tools that Fleet agents can use during runs.
...
- Fleet server starts one worker process per installed plugin
...
Receives one typed Fleet domain event.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Fleet secret provider system rather than a plain text input
...
1. Core Fleet data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Fleet objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Fleet product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Fleet later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Fleet secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm fleet plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Fleet server. This is a normative requirement, not optional.
...
For developing a plugin against a running Fleet instance:

- The operator installs the plugin from a local path: `pnpm fleet plugin install ./path/to/plugin`
...
Fleet should not implement a generic in-process hook bag modeled directly after local coding tools.

Fleet should implement:
...
That is the complete target design for the Fleet plugin system.

...
This spec defines how Fleet actually runs agents while staying runtime-agnostic.
...
1. Fleet is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Fleet manages lifecycle, and on exit Fleet parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `fleet.skill` (shared Fleet skill text block)
...
2. Preferred transport is env vars (`FLEET_*`) injected at runtime.

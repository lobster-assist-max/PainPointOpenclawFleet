...
Maintainer runbook for shipping Fleet across npm, GitHub, and the website-facing changelog surface.
...
Fleet uses calendar versions that still fit semver syntax:
...
2. **npm** — `fleet` and public workspace packages are published
...
# or
npx fleet@canary onboard --data-dir "$(mktemp -d /tmp/fleet-canary.XXXXXX)"
```
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Fleet. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
FLEET_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
FLEET_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary FLEET_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable FLEET_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f fleet_version=canary
gh workflow run release-smoke.yml -f fleet_version=latest
```

Minimum checks:

- `npx fleet@canary onboard` installs

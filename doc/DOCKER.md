...
Run Fleet in Docker without installing Node or pnpm locally.
...
For authenticated deployments, set one canonical public URL and let Fleet derive auth/callback defaults:
...
- Adapter environment checks in Fleet will surface missing auth/CLI prerequisites.
...
- `npx fleet onboard --yes` completes

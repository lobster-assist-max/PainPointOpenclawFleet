...
ClipHub is the public registry where people share, discover, and download Fleet company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Fleet what a package registry is to a programming language. Fleet already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Fleet — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Fleet instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Fleet spec. It contains:
...
**Install (fresh start):**
```
fleet install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Fleet instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Fleet, export your company as a template, then publish:

```
fleet export --template my-company
fleet publish cliphub my-company
```
...
ClipHub is a **separate service** from Fleet itself. Fleet is self-hosted; ClipHub is a hosted registry that Fleet instances talk to.
...
| **Fleet CLI** | `fleet install`, `fleet publish`, `fleet cliphub sync` — built into Fleet |
| **Fleet UI** | "Browse ClipHub" panel in the Fleet web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Fleet) |
| Backend | TypeScript + Hono (consistent with Fleet) |
| Database | PostgreSQL |
...
4. Run `fleet install cliphub:acme/lean-saas-shop`
5. Fleet creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Fleet until it works well
2. Export: `fleet export --template my-agency`
3. Publish: `fleet publish cliphub my-agency`
...
3. Install just that agent: `fleet install cliphub:acme/senior-python-eng --agent`
...
ClipHub is **not required** to use Fleet. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Fleet what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `fleet install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Fleet web UI
- [ ] `fleet cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Fleet's own philosophy)

...
Fleet supports two runtime modes with different security profiles.
...
```sh
pnpm fleet allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Fleet emits a one-time claim URL at startup:

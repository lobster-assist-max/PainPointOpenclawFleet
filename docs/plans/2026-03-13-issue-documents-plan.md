...
Add first-class **documents** to Fleet as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Fleet workflow/docs.
...
Update the Fleet agent workflow so planning no longer edits the issue description.

...
Build a custom adapter to connect Fleet to any agent runtime.
...
2. Build environment with `buildFleetEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `fleet run --watch` using `picocolors`.
...
Make Fleet skills discoverable to your agent runtime without writing to the agent's working directory:

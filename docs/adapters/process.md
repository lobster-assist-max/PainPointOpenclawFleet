...
- Running a Python script that calls the Fleet API
...
1. Fleet spawns the configured command as a child process
2. Standard Fleet environment variables are injected (`PAPERCLIP_AGENT_ID`, `PAPERCLIP_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Fleet API and perform work.

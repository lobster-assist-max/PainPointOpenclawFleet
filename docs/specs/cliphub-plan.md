# ClipHub: Marketplace for Fleet Team Configurations

> The "app store" for whole-company AI teams — pre-built Fleet configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Fleet-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Fleet company (agents, projects, governance) |
...
A complete Fleet company configuration:
...
Single-agent configurations designed to plug into a Fleet org:
...
Portable skill files that any Fleet agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Fleet's skill loading system
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Fleet company
...
| `POST` | `/api/listings/:id/install` | Install to Fleet company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Fleet company (or create new)
  → ClipHub API calls Fleet API to:
  ...
  → Redirect to Fleet dashboard with new team running
```
...
### 5.3 Creator: Export from Fleet → Publish

```
Running Fleet company → "Export as Blueprint" (CLI or UI)
  → Fleet exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Fleet brand blue/purple)
...
| Install flow | One-click deploy to Fleet company |
...
{
  "targetCompanyId": "uuid",         // Existing Fleet company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Fleet
- **Backend**: Node.js API (or extend Fleet server)
- **Database**: Postgres (can share Fleet's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Fleet auth (or OAuth2)

### 9.2 Integration with Fleet

ClipHub can be:
- **Option A**: A separate app that calls Fleet's API to install blueprints
- **Option B**: A built-in section of the Fleet UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Fleet UI and API.
...
- [ ] Install flow: blueprint → Fleet company

...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Fleet API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "fleetApiUrl",
                  String(config.fleetApiUrl ?? config.paperclipApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "fleetApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://fleet.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "fleet"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway bots. Fleet persists a device key during onboarding so pairing approvals

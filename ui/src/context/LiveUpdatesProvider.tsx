...
function handleLiveEvent(
  ...
  // ── Fleet Monitor events ──────────────────────────────────────────────
  if (event.type.startsWith("fleet.")) {
    // Always refresh fleet status so Dashboard stays current
    ...
    if (event.type === "fleet.bot.connected" || event.type === "fleet.bot.disconnected") {
      ...
      gatedPushToast(gate, pushToast, "fleet-connection", {
        title: `${emoji} ${name} ${verb}`,
        tone: event.type === "fleet.bot.connected" ? "success" : "warn",
      });
    }
    return;
  }

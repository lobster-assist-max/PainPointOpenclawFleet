...
/**
 * Time since a date. Keeps second-level precision ("34s ago") — valuable on the
 * live fleet surfaces (session tail, alerts, notifications) where the general
 * `@/lib/timeAgo` copy would show a coarser "just now". Adds week/month tiers so
 * a long-ago timestamp (a notification persisted for weeks, an old chat message)
 * reads "3w ago" / "2mo ago" instead of an unbounded "90d ago".
 */

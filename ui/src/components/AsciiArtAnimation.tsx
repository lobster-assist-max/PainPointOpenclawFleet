...
type LobsterSprite = (typeof LOBSTER_SPRITES)[number];
...
function spriteSize(sprite: LobsterSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = LOBSTER_SPRITES[(baseCol + baseRow) % LOBSTER_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = LOBSTER_SPRITES[Math.floor(Math.random() * LOBSTER_SPRITES.length)]!;

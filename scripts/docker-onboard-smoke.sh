...
IMAGE_NAME="${IMAGE_NAME:-fleet-onboard-smoke}"
...
FLEET_DEPLOYMENT_MODE="${FLEET_DEPLOYMENT_MODE:-${PAPERCLIP_DEPLOYMENT_MODE:-authenticated}}"
FLEET_DEPLOYMENT_EXPOSURE="${FLEET_DEPLOYMENT_EXPOSURE:-${PAPERCLIP_DEPLOYMENT_EXPOSURE:-private}}"
FLEET_PUBLIC_URL="${FLEET_PUBLIC_URL:-${PAPERCLIP_PUBLIC_URL:-http://localhost:${HOST_PORT}}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@fleet.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-fleet-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e FLEET_DEPLOYMENT_MODE="$FLEET_DEPLOYMENT_MODE" \
      -e FLEET_DEPLOYMENT_EXPOSURE="$FLEET_DEPLOYMENT_EXPOSURE" \
      -e FLEET_PUBLIC_URL="$FLEET_PUBLIC_URL" \
      -e FLEET_HOME="/fleet" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "fleet@${FLEET_VERSION}" auth bootstrap-ceo --data-dir "$FLEET_HOME" --base-url "$FLEET_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$FLEET_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$FLEET_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$FLEET_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$FLEET_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$FLEET_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$FLEET_PUBLIC_URL/api/companies")"
...
echo "    Public URL: $FLEET_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $FLEET_DEPLOYMENT_MODE/$FLEET_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/fleet-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$FLEET_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $FLEET_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$FLEET_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $FLEET_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"

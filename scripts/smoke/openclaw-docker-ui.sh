...
FLEET_HOST_PORT="${FLEET_HOST_PORT:-${PAPERCLIP_HOST_PORT:-3100}}"
FLEET_HOST_FROM_CONTAINER="${FLEET_HOST_FROM_CONTAINER:-${PAPERCLIP_HOST_FROM_CONTAINER:-host.docker.internal}}"
...
detect_fleet_base_url() {
  ...
  for candidate in "$FLEET_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${FLEET_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${FLEET_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${FLEET_HOST_PORT}
  (Do not use http://127.0.0.1:${FLEET_HOST_PORT} inside the container.)

...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Fleet data dir to use
  FLEET_HOST=127.0.0.1        Host passed to the onboarded server
  FLEET_PORT=3232             Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/fleet-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/fleet-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export FLEET_HOST="${FLEET_HOST:-${PAPERCLIP_HOST:-127.0.0.1}}"
export FLEET_PORT="${FLEET_PORT:-${PAPERCLIP_PORT:-3100}}"
export FLEET_OPEN_ON_LISTEN="${FLEET_OPEN_ON_LISTEN:-${PAPERCLIP_OPEN_ON_LISTEN:-false}}"
...
env \
  ...
  HOST="$FLEET_HOST" \
  PORT="$FLEET_PORT" \
  FLEET_OPEN_ON_LISTEN="$FLEET_OPEN_ON_LISTEN" \
  pnpm fleet onboard --yes --data-dir "$PC_DATA"

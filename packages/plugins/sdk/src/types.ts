/**
 * Core types for the Fleet plugin worker-side SDK.
 *
...
export interface ScopeKey {
  /** What kind of Fleet object this state is scoped to. */
...
/**
 * `ctx.events` — subscribe to and emit Fleet domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Fleet domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Fleet secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Fleet secret provider (e.g. `"MY_API_KEY"`).
   *

...
Kitchen Sink is the first-party reference plugin that demonstrates nearly the full currently implemented Fleet plugin surface in one package.
...
pnpm fleet plugin install ./packages/plugins/examples/plugin-kitchen-sink-example
```

Or install it from the Fleet plugin manager as a bundled example once this repo is built.
...
- Some SDK-defined host surfaces still depend on the Fleet host wiring them visibly; this package aims to exercise the currently mounted ones and make the rest obvious.

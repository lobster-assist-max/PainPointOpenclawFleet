...
type FleetSprite = (typeof FLEET_SPRITES)[number];
...
function spriteSize(sprite: FleetSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = FLEET_SPRITES[(baseCol + baseRow) % FLEET_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = FLEET_SPRITES[Math.floor(Math.random() * FLEET_SPRITES.length)]!;

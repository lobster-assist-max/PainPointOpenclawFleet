...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.FLEET_OPENCODE_COMMAND === "string" &&
    process.env.FLEET_OPENCODE_COMMAND.trim().length > 0
      ? process.env.FLEET_OPENCODE_COMMAND.trim()
...
const VOLATILE_ENV_KEY_PREFIXES = ["FLEET_", "PAPERCLIP_", "npm_", "NPM_"] as const;

...
Use when:
- You want Fleet to run Pi (the AI coding agent) locally as the agent runtime
...
- Fleet requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/fleet/ and resumed with --session.

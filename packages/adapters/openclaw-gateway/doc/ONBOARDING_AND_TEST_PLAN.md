...
## Scope
This plan is now **gateway-only**. Fleet supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Fleet skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Fleet task.

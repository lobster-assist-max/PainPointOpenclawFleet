...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://fleet:fleet@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "fleet");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://fleet:fleet@127.0.0.1:${preferredPort}/fleet`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://fleet:fleet@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "fleet");
    return {
      connectionString: `postgres://fleet:fleet@127.0.0.1:${port}/fleet`,
  ...
  const adminConnectionString = `postgres://fleet:fleet@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "fleet");

  return {
    connectionString: `postgres://fleet:fleet@127.0.0.1:${selectedPort}/fleet`,

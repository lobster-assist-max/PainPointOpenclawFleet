...
async function createTempDatabase(): Promise<string> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "fleet-db-client-"));
  ...
  const adminUrl = `postgres://fleet:fleet@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminUrl, "fleet");
